...
def classif_roots(V,list_roots):
    """
...
def weight_as_list(weight):
    if weight.as_list!=None:
        return weight.as_list
    elif weight.as_list_of_list!=None:
        return weight.as_list_of_list[0]
    else:
        raise ValueError(f"Weight {weight} has no attribute .as_list or .as_list_of_list")
        
#TODO: define smaller polynomial rings?

def map_pi(V,weights_domain,roots_domain,method):
    ...
    if V.type=="kron":
        degree=len(V.G)-1
    elif V.type in ["boson","fermion"]:
        degree=V.nb_part
    weights_domain
    v = point_vect(weights_domain, V, ring, bounds=(-1000, 1000))
    ...
    if V.type in ["boson","fermion"]:
        classified_roots*=V.nb_part #
    Res=[0 for i in range(len(v))] #null vector
    for i, weight in enumerate(weights_domain):
        ...
        for tuple_roots in compatible_tuples_of_roots:
            target_weight=[tuple_roots[k].i for k in range(degree)]
            ...
            if V.type in ["fermion","boson"]:
                if V.type=="fermion":
                    coef=(-1)**(Permutation(target_weight).length)#taking into account -1 sign in reordering wedge product
                target_weight.sort()
                if V.type=="fermion":
                    compute=sum([target_weight[i]==target_weight[i+1] for i in range(degree-1)])==0
                target_weight=Weight(V.G,as_list_of_list=[target_weight])
            elif V.type=="kron":
                target_weight=Weight(V.G,as_list=target_weight+[0])
            for root in tuple_roots:
                if root.i!=root.j:
                    coef*=variable(ring_or_gens= bigring, name_or_weight=root,seed="u")
            if compute: #in fermionic cases v\wedge v=0:
                Res[target_weight.idx(V)]+=v[weight.idx(V)]*coef
    return Res
    

def eq_fibre_pi(V,ineq,method):
    ...
    for weight in positive_weights:
        Res.append(map_formula[weight.idx(V)])
    return Res
    
def Groebner_basis_brut(V,ineq,method):
    """
...
def is_fiber_singleton(V,ineq,method):
   """
...
def is_fiber_singleton_reorder(ineq,V,method):
    """
...
def long_calculation(Liste, function, lim, extra_arguments):
    """
