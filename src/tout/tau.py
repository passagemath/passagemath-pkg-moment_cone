...
class Tau:
    ...
    def orthogonal_all_roots(self,V: Representation) -> list[Root]:
        """
...
def find_1PS(V: Representation, quiet: bool = False) -> Sequence["Tau"]:
    ...
    if V.type=='kron' :
        ...
        sub_rep=[Representation(LinGroup([1]+list(p)),'kron') for p in Partition(V.G[1:]).all_subpartitions()][1:] #[1:] excludes 1... 1
        for small_V in sub_rep:  
            umax=V.u_max(small_V)
        ...
        list_partS=[p for p in Partitions(V.G.rank)][1:] #[1:] excludes n, so S is the center of G
        for partS in list_partS :
            ...
            for H in find_hyperplanes_reg(weights, Vred, umax,exp_dim, sym):
                #if partS == Partition([3,2]):
                #    print('H,umax,sym,exp_dim,Vred',H,umax,sym,exp_dim,Vred)
                ...
                #print(l1,l2)
                Liste_1PS+=[Tau.from_flatten(l1,V.G),Tau.from_flatten(l2,V.G)]

    Liste_1PS=list(set(Liste_1PS)) # Suppressing the repetitions
    
    # Checking that the candidates really give a candidate
    for tau in Liste_1PS:
        print('tau,orth weights',tau,tau.orthogonal_weights(V))
        
    ListeRes=[tau for tau in Liste_1PS if check_hyperplane_dim([chi for chi in tau.orthogonal_weights(V)], V.dim_cone-1)] # TODO : liste de chi utile ?
    
    return list(set(ListeRes)) # Suppressing the repetitions
